...
export class MediaStreamService {
  ...
  private readonly _isMuted = new BehaviorSubject<boolean>(true);
  private readonly _isCameraOff = new BehaviorSubject<boolean>(true);
  ...
  toggleCamera(): boolean {
    ...
    const cameraEnabled = !newCameraOff;
    ...
    console.log(`📹 Camera toggled: ${cameraStatus}`, {
  ...
  stopScreenShare(): void {
    this._screenStream.value?.getTracks().forEach((t) => {
      t.stop();
    });
  ...
  stopAll(): void {
    this._localStream.value?.getTracks().forEach((t) => {
      t.stop();
    });
